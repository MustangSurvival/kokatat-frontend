...
export default class ProductDetails {
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', (err, response) => {
            ...
            this.updateInventoryMessaging(response.data.v3_variant_id, response.data.instock);
        });
    }
    ...
    showMessageBox(message) {
        ...
        const $availabilityBox = $('.availability-message');
    ...
    disableSelectOptionAttribute($attribute, behavior, outOfStockMessage) {
        ...
        if (behavior === 'hide_option') {
            ...
            // + outOfStockMessage //displays out-of-stock message in parens...this is commented out to prevent emtpy parens showing as customer doesn't want out-of-stock message being displayed
            );
        }
    }
    ...
    updateInventoryMessaging(variantId, inStock) {
        if (inStock) {
            if (typeof variantId !== 'undefined' && variantId !== null && typeof this.context.variantData !== 'undefined' && this.context.variantData.length) {
                for (let i = 0; i < this.context.variantData.length; i++) {
                    if (this.context.variantData[i].hasOwnProperty(variantId)) {
                        if (this.context.variantData[i][variantId] > 0 && document.querySelector('.availability-message .eyebrow') !== null || this.context.productData.category[0] === 'Accessories/repair') {
                            document.querySelector('.availability-message .eyebrow').innerHTML = 'In Stock';
                        } else if (this.context.productData.title.includes("Warranty")) {
                            document.querySelector('.availability-message .eyebrow').innerHTML = 'In Stock';
                        } else {
                            document.querySelector('.availability-message .eyebrow').innerHTML = 'Available in 20 Business Days';
                        }
                        break;
                    }
                }
            } else if (document.querySelector('.availability-message .eyebrow') !== null || this.context.productData.category[0] === 'Accessories/repair') {
                document.querySelector('.availability-message .eyebrow').innerHTML = 'In Stock';
            }
        }
    }
